...
async function viewShow() {
    const res = await axios.get(baseUrl + '/shopping/api/v1/article');
    return res.data;
}

async function createCart() {
    const res = await axios.post(baseUrl + '/shopping/api/v1/cart/', {});
    return res.headers.location;
}

async function getOrderByCartRef(cartId) {
    const res = await axios.post(baseUrl + '/checkout/api/v1/cart/' + cartId + '/order', {})
    return res.headers.location;
}
...
async function updateOrder(order, firstName, lastName, street, zipCode, city) {
    await axios.patch(order, {firstName, lastName, street, zipCode, city});
}
...
async function getPaymentByOrder(orderId) {
    const res = await axios.get(baseUrl + '/payment/api/v1/order/' + orderId);
    return res.data.payment;
}

async function getPayment(paymentId) {
    const res = await axios.get(baseUrl + '/payment/api/v1/payment/' + paymentId);
    return res.data;
}

async function doPayment(paymentId) {
    await axios.post(baseUrl + '/payment/api/v1/payment/' + paymentId + '/pay', {});
}
